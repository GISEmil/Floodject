...
\label{ch:implementation} % For referencing the chapter elsewhere, use \autoref{ch:introduction} 
...
\paragraph{Setting up a local scripting environment:} Instead of working directly on the server, and working with PyWPS we decided to create a local working environment using GRASS with Python, on our own machines. This was done to simplify the workflow and make sure that  we did not have to update the server whenever changes were made to the code. \\
...
The first action we took was to change the default path of our application in the server. Instead of using \textit{/var/www/html}, we started using \textit{/var/www/html/FlaskApp/FlaskApp}. This path will be referred to as the root url. After doing that, we create the script that will perform the functionalities of the application. That script is named \texit{\_\_init\_\_.py}. Using Flask allows us to connect to various html scripts by using  only one main script. Depending on the URL of the page the user wants to get to, the corresponding HTML script is called and displayed on the user's screen. For example, if the user visits our homepage, which is set by the \textit{\@app.route()}, then the \textit{'index.html'} will be initialized and displayed on his screen:\\
...
The first action the user must perform in order to begin using the application is to upload a DEM. When the user clicks on the FLOOD button on our starting page, they get redirected to the \texit{root url/upload} section of the application. The way that section is working is that it expects a file to be posted. When that happens, it saves it to a pre-designated folder on the server. Right after that, we create a copy of the uploaded file and convert it from \texit{.tif} to \texit{.png}. The reason behind that conversion is that we need to display the uploaded elevation model on a map for the user to see, which is vital to the next steps of the application. In order to be able to overlay an object on top of a map using the leaflet mapping library, then that object has to be of \texit{.png} or \texit{.jpeg} format. All the aforementioned functions are included in the script below.\\
...
\section{Phase 4 - Added functionalities}
\label{ch:phase4}
...
\begin{figure}[h!]
  \myfloatalign
  \subfloat[Showing the placement of outlets before spillover.]
  {\label{fig:outletanala}
  {\includegraphics[width=0.75\linewidth]{gfx/Phase_4/outletanal.png}}} \\ \quad
  \subfloat[Showing the placement of outlets after spillover.]
  {\label{fig:outletanalb}
   \includegraphics[width=0.75\linewidth]{gfx/Phase_4/outletanal2.png}} \\
  \caption{Showing the relationship between watershed outlets and the flooding of water.}
  \label{fig:outletanal}
\end{figure}

\autoref{fig:outletanal} shows a relatively large take over of the flood. Since this flood extent is advancing regardless of watershed position, the outlet (red cross') have no meaning for the further investigation of predicting pour-point positions.
...
\paragraph{Step 2 – Select Source:}
...
We decided to add the possiblity for the user to choose between two different types of flooding at this stage. Either to do a simple flooding simulation, or a more advanced flood.
...
The request we will perform is a standard jQuery async request to the PyWPS by using \textit{\$.get()}. This is encased in a \texit{\$.when} and a \textit{\$.done}function as follows: \textit{\$.when(\$.get()).done();}

This sets the script up in such a way that, when the async request is done, it launches another script. 
The \texit{\$.when} and async request code looks like the following:
...
What this does is to get the XML document returned from PyWPS, traverse it and for each result, store it in a variable called \texit{outputimagearray}.
